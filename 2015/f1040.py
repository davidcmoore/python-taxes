...
from f8801_2016 import F8801_2016
...
class F1040(Form):
    STD_DED = [6300, 12600, 6300, 9250, 12600]
    EXEMPTION = 4000
    BRACKET_RATES = [0.10, 0.15, 0.25, 0.28, 0.33, 0.35, 0.396]
    BRACKET_LIMITS = [
        [9225, 37450, 90750, 189300, 411500, 413200],   # SINGLE
        [18450, 74900, 151200, 230450, 411500, 464850], # JOINT
        [9225, 37450, 75600, 115225, 205750, 232425],   # SEPARATE
        [13150, 50200, 129600, 209850, 411500, 439000], # HEAD
        [18450, 74900, 151200, 230450, 411500, 464850], # WIDOW
    ]
    SS_MAX = 7347
    ...
    def __init__(f, inputs={}):
        ...
        f8801_2016 = F8801_2016(inputs, f, f6251, f8801, sd)
        f.addForm(f8801_2016)
    ...
    def deduction_for_exemptions(f, status):
        THRESH = [258250, 309900, 154950, 284050, 309900]
        w = {}
