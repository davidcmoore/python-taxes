...
class F8801_2016(Form):
    """Form 8801, Credit for Prior Year Minimum Tax

    This form is for 2016, but since it depends on values
    from 2015, it is generated along with 2015's
    forms. Line 21 from this form must be manually entered into
    2016's input dictionary.
    """
    def __init__(f, inputs, f1040, f6251, f8801, sched_d):
        super(F8801_2016, f).__init__(inputs)
        f['1'] = f6251.rowsum(['1', '6', '10'])
        f['2'] = f6251.rowsum(['2', '3', '4', '5', '7', '8', '9', '12', '13'])
        # TODO: Line 3, minimum tax credit net operating loss deduction
        f['4'] = max(0, f['1'] + f['2'] + f['3'])
        assert(inputs['status'] != FilingStatus.SEPARATE or f['4'] <= 246250)
        ...
        if f['4'] == 0:
            ...
            if f['10'] == 0:
                ...
                if (f1040['13'] and not sched_d.mustFile()) or f1040['9b'] or \
                    ...
                    f['34'] = f1040.BRACKET_LIMITS[inputs['status']][1]
                    f['35'] = cg_worksheet['7']
                    f['36'] = max(0, f['34'] - f['35'])
                    f['37'] = min(f['27'], f['28'])
                    f['38'] = min(f['36'], f['37'])
                    f['39'] = f['37'] - f['38']
                    f['40'] = f1040.BRACKET_LIMITS[inputs['status']][-1]
                    f['41'] = f.get('36')
                    f['42'] = cg_worksheet['7']
                    f['43'] = f.rowsum(['41', '42'])
        ...
        f['17'] = f.get('15')
        f['18'] = f['16'] - f['17']
        f.comment['19'] = '2015 credit carryforward'
        f['19'] = f8801.get('26')
        f.comment['21'] = 'Accum. credit (enter in 2016 inputs as \'prior_amt_credit\')'
        f['21'] = max(0, f['18'] + f['19'] + f['20'])
        if f['21']:
            f.must_file = True

    def title(self):
        return 'Form 8801 (for 2016 filing)'
