...
class F6251(Form):
    ...
    EXEMPTIONS = [71700, 111700, 55850, 71700, 111700]
    EXEMPT_LIMITS = [510300, 1020600, 510300, 510300, 1020600]
    RATE_CHANGE = [194800, 194800, 97400, 194800, 194800]
    def __init__(f, inputs, f1040, sched_a, sched_d, sched_d_re):
        ...
        f['1'] = f1040.get('11b')
        if f['1'] == 0:
            f['1'] = f1040['8b'] - f1040['9'] - f1040['10']
        f['2a'] = sched_a.get('7') if sched_a else f1040.get('9')
        if 's1_1' in f1040:
            # TODO: refunds from 1040 schedule 1, line 8
            f['2b'] = -f1040['s1_1']
        ...
        if f['6'] <= 0:
            ...
            f['6'] = 0
            f['10'] = f1040['12a'] + f1040['s2_2'] - f1040['s3_1']
            return
