...
def compute_with_income(template, income, capital_gains):
    ...
    ss = min(SOCIAL_SECURITY_MAX, income)
...
for x in xrange(0, max_income, step):
    ...
    rate         = float(fnext['61'] - fbase['61']) / inc
    capgain_rate = float(fcapgain['61'] - fbase['61']) / inc

    print('%6d %6d %5.3f %5.3f' % (x, fbase['61'], rate, capgain_rate))
    incomes.append(x)
    rates.append(rate)
