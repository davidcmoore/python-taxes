...
class F8801_2015(Form):
    """Form 8801, Credit for Prior Year Minimum Tax

    This form is for 2015, but since it depends on values
    from 2014, it is generated along with 2014's
    forms. Line 21 from this form must be manually entered into
    2015's input dictionary.
    """
    def __init__(f, inputs, f1040, f6251, f8801, sched_d):
        super(F8801_2015, f).__init__(inputs)
        ...
        assert(inputs['status'] != FilingStatus.SEPARATE or f['4'] <= 242450)
        ...
        if f['4'] == 0:
            ...
            if f['10'] == 0:
                ...
                if (f1040['13'] and not sched_d.mustFile()) or f1040['9b'] or \
                    ...
                    f['34'] = f6251.CAP_GAIN_LIMITS[inputs['status']]
                    f['35'] = cg_worksheet['7']
        ...
        f['17'] = f.get('15')
        f['18'] = f['16'] - f['17']
        f.comment['19'] = '2014 credit carryforward'
        f['19'] = f8801.get('26')
        f.comment['21'] = 'Accum. credit (enter in 2015 inputs as \'prior_amt_credit\')'
        f['21'] = max(0, f['18'] + f['19'] + f['20'])
        if f['21']:
            f.must_file = True

    def title(self):
        return 'Form 8801 (for 2015 filing)'
