...
class CA540sca(Form):
    STD_DED = [5706, 11412, 5706, 11412, 11412]
    ...
    def __init__(f, inputs, f1040, f1040sa):
        ...
        f['1B_B10'] = f.rowsum(['1A_B1z', '1A_B[2-6]b', '1A_B7a', '1B_B[1-7]', '1B_B9a', '1B_B9b[1-3]'])
        f['1B_C10'] = f.rowsum(['1A_C1z', '1A_C[2-5]b', '1A_C7a', '1B_C2a', '1B_C[1-6]', '1B_C9a'])
        ...
        f.comment['1C_B27'] = 'Income Subtractions'
        f['1C_B27'] = f['1B_B10'] - f['1C_B26']
        f.comment['1C_C27'] = 'Income Additions'
        ...
        f.comment['2_B17'] = 'Itemized Deduction Subtractions'
        f['2_B17'] = f.rowsum(['2_B7', '2_B10', '2_B1[4-6]'])
        f.comment['2_C17'] = 'Itemized Deduction Additions'
        ...
        f.comment['2_29'] = 'Itemized Deduction w/ limitations'
        ...
        if inputs.get('can_be_dependent', False):
            std = min(std, max(1350, f1040.earned_income()))
            if f1040.unearned_income() > 2700:
                raise RuntimeError('TODO: Kiddie Tax, form 3800')

        if f['2_29'] > std:
            f.comment['2_30'] = 'Itemized Deduction'
            f['2_30'] = f['2_29']
            f.must_file = True
        else:
            f.comment['2_30'] = 'Standard Deduction'
            f['2_30'] = std

        if f['1C_B27'] or f['1C_C27']:
            f.must_file = True
    ...
    def itemized_deductions_worksheet(f, inputs, f1040, f1040sa):
        ...
        w = {}
        w['1'] = f['2_28']
        if f1040['11b'] <= LIMITS[inputs['status']]:
            return w['1']
