...
class F6251(Form):
    ...
    EXEMPTIONS = [88100, 137000, 68500, 88100, 137000]
    EXEMPT_LIMITS = [626350, 1252700, 626350, 626350, 1252700]
    RATE_CHANGE = [239100, 239100, 119550, 239100, 239100]
    def __init__(f, inputs, f1040, sched_a, sched_d, sched_d_re):
        ...
        f['1a'] = f1040['14'] - f1040['s1a_37']
        f['1b'] = f1040['11b'] - f['1a']
        f['2a'] = sched_a.get('7') if sched_a else f1040.get('12e')

        if 's1_1' in f1040:
            # TODO: refunds from 1040 schedule 1, line 8z
            f['2b'] = -f1040['s1_1']

        if (f.rowsum(['2[c-z]', '3']) or 0) < 0:
            f.must_file = True

        f['4'] = f.rowsum(['1b', '2[a-z]', '3'])
        ...
        if f['6'] <= 0:
            ...
            f['6'] = 0
            f['10'] = max(0, f1040['16'] + f1040['s2_1z'] - f1040['s3_1'])
            return

        # TODO: capital gains refigured for the AMT
        # TODO: form 2555
        if (f1040['7a'] and not sched_d.mustFile()) or f1040['3a'] or \
