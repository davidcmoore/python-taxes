...
class Form(object):
    ...
    def printPropDiffs(self, prev, prefix=""):
        found_diff = False
        for k in set(list(self.props.keys()) + list(prev.keys())):
            val = self.props.get(k)
            if val == prev.get(k):
                continue
            found_diff = True
            if type(val) == bool:
                if val:
                    print(f'{prefix}begin {k}')
                else:
                    print(f'{prefix}end {k}')
            else:
                print(f'{prefix}begin {k} == {val}')
        return found_diff
