from form import Form, FilingStatus

MEDICARE_RATE = 0.0145

class F8959(Form):
    """Form 8959, Additional Medicare Tax"""
    THRESHOLDS = [200000, 250000, 125000, 200000, 200000]
    def __init__(f, inputs, sse):
        super(F8959, f).__init__(inputs)
        f['1'] = f.spouseSum(inputs, 'wages_medicare')
        # TODO: forms 4137, 8919
        f['4'] = f.rowsum(['1', '2', '3'])
        if f.get('4'):
            f['5'] = f.THRESHOLDS[inputs['status']]
            f['6'] = max(0, f['4'] - f['5'])
            f['7'] = f['6'] * .009

        if inputs['status'] == FilingStatus.JOINT:
            if sse[0].mustFile() or sse[1].mustFile():
                f['8'] = max(0, (sse[0]['6'] or 0) + (sse[1]['6'] or 0))
        else:
            if sse.mustFile():
                f['8'] = max(0, sse['6'])
        if f.get('8'):
            f['9'] = f.THRESHOLDS[inputs['status']]
            f['10'] = f.get('4')
            f['11'] = max(0, f['9'] - f['10'])
            f['12'] = max(0, f['8'] - f['11'])
            f['13'] = f['12'] * .009 or None

        if f.get('14'):
            f['15'] = f.THRESHOLDS[inputs['status']]
            f['16'] = max(0, f['14'] - f['15'])
            f['17'] = f['16'] * .009 or None

        f.comment['18'] = 'Total Additional Medicare Tax'
        f['18'] = f.rowsum(['7', '13', '17'])
        f.props['additional_medicare_tax'] = (f['18'] > 0)

        f['19'] = f.spouseSum(inputs, 'medicare_withheld')
        f['20'] = f.get('1')
        f['21'] = f['20'] * MEDICARE_RATE or None
        if f['19'] or f['21']:
            f['22'] = max(0, f['19'] - f['21'])

        f.comment['24'] = 'Total Additional Medicare Tax withholding'
        f['24'] = f.rowsum(['22', '23'])

        if f.get('18') or f.get('24'):
            f.must_file = True

    def title(self):
        return 'Form 8959'
